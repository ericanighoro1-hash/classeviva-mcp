...
def _login_spaggiari(username: str, password: str):
    login_url = "https://web.spaggiari.eu/rest/v1/auth/login"
    headers = {"Z-Dev-Apikey": "+apRest4198106="}
    body = {"ident": username, "passa": password}
    
    response = requests.post(login_url, json=body, headers=headers)
    if response.status_code != 200:
        raise HTTPException(status_code=401, detail="Credenziali ClasseViva non valide")
